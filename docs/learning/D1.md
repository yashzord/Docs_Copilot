...
| `frontend/app/api/chat/route.ts` | the proxy to FastAPI | web.md 10 |
...
Cognito login replaces it in D2.
...
Not started. Will add: a Dockerfile for each half and one `compose.yaml`
that starts both with `docker compose up`. Not needed for the demo: two
terminals do the same job.
