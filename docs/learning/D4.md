...
flowchart LR
    ...
    H -->|how things relate| G[graph___search_graph] --> L[Lambda<br/>about 40 lines of logic<br/>plus a self-check] --> K2[GraphRAG Knowledge Base]
...
## After D4: fresh start and a real-browser test (2026-09-11)

All test data was wiped (568 Memory events, 88 long-term records, the
bucket, both indexes, the unused role `...KnowledgeBase_mq0oz`). Then the
app was driven in a real browser with Playwright: upload, three kinds of
question, the sidebar. Command-line tests had passed; the browser found
what they could not see:

| Found | Fix |
|---|---|
| wiped conversations still listed, empty | `sessions.py` hides conversations with no events (+1 test) |
| answers showed raw `**markdown**` | `lib/markdown.ts` + `Markdown` in `Chat.tsx` (+4 tests), `web.md` 13.7 |
| a web page read returned only the cookie banner, and the answer still cited the page | prompt rule 1: read the whole page, get past consent banners, admit failure |
| citations as bare numbers (`hour1.`) | prompt rule 3: an exact example |
| browser steps showed as "Used browser" | only "Opened <url>" is shown; graph search reads "Searched the knowledge graph for ..." |

Verified in the browser afterwards: markdown rendered, the graph question
answered from `aws.md` through `graph___search_graph`, 0 console errors.
